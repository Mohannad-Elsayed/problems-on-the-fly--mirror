...
void solve() {
    ...
    print(zeros);
    int ans = zeros;
    for (int i = x; i <= sum; i+=x) {
        ans = combinatorics::add(combinatorics::mul(combinatorics::nCr(sum, i), zeros), ans);
        ans = combinatorics::add(combinatorics::nCr(sum, i), ans);
    }
    cout << ans;
}